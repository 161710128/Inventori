...
class PengirimanBarang extends CI_Controller{
	...
	public function proses_tambahEdit(){  
		...
		for($i = 0; $i < $jumlah_barang_diterima; $i++){
			...
			$data_detail_terima[$i]['total_stok'] = $this->input->post('total_stok_hidden')[$i]; 
		...
		if($this->m_stok_alat->tambahDetailPengiriman($data_detail_terima)){
			for ($i=0; $i < $jumlah_barang_diterima ; $i++) { 
				$this->m_stok_alat->min_stok($data_detail_terima[$i]['total_stok'], $data_detail_terima[$i]['kode_alat']) or die('gagal min stok');
			}
	...
	public function detailHapus($kode_komponen)
	{
		...
		$this->load->view('packing/transaksiPacking/hapusDetailPengiriman', $this->data);
	}
    ...
    { 
        ...
        if ($this->m_stok_alat->hapusDetailPengiriman($id_detail)) {
            
            $this->m_stok_alat->plus_stok($total_stok,$kode_komponen) or die('gagal min stok'); 
            redirect('Packing/PengirimanBarang');
        }
    }
