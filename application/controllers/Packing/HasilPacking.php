<?php 
class HasilPacking extends CI_Controller{
	public function __construct(){
		parent::__construct();
		date_default_timezone_set('Asia/Jakarta'); 
		$this->data['aktif'] = 'HasilPacking';
	...
	public function index(){
		...
		$this->load->view('packing/transaksiPacking/lihat', $this->data);
	}
	...
	public function tambah(){ 
		...
		if(empty($dariDB)){
    		...
    		$this->load->view('packing/transaksiPacking/tambah_alat', $this->data);
		}else{
	        $dariDB = $this->m_stok_alat->cekkodetransaksi();
	...
	public function proses_tambah(){
		...
		if(empty($countDuplicate)){ 
			...
			redirect('Packing/HasilPacking'); 
		} else { 
			$this->session->set_flashdata('errorStok', 'Tanggal Sudah Ada !'); 
			redirect('Packing/HasilPacking/tambah');  
		} 
	}
	...
	public function tambahEdit($kode_transaksi){    
		...
		$this->data['titleHead'] = 'Tambah | Produksi';
		$this->data['all_barangHFNC'] = $this->m_stok_alat->stok_alat_packing(); 
		...
		$this->load->view('packing/transaksiPacking/tambahEdit', $this->data);
	}
	...
	public function keranjang_barang(){
		$this->load->view('packing/transaksiPacking/keranjang');
	} 
	...
	public function proses_tambahEdit(){  
		...
		for($i = 0; $i < $jumlah_barang_diterima; $i++){
			...
			$data_detail_terima[$i]['serial_number'] = $this->input->post('serialnumber_hidden')[$i]; 
			$data_detail_terima[$i]['total_stok'] = 1; 
			...
			$data_detail_terima[$i]['tanggal'] = $new_date_format; 
		}

		if($this->m_stok_alat->tambahDetail($data_detail_terima)){
			for ($i=0; $i < $jumlah_barang_diterima ; $i++) { 
				$this->m_stok_alat->plus_stok($data_detail_terima[$i]['total_stok'], $data_detail_terima[$i]['kode_alat']) or die('gagal min stok');
			}
	...
	public function detail($kode_transaksi){
		...
		$this->load->view('packing/transaksiPacking/detail', $this->data);
	}
	...
	public function detailHapus($kode_komponen)
	{
		...
		$this->load->view('packing/transaksiPacking/hapusDetail', $this->data);
	}
