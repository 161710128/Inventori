<?php 
class MasterHasil extends CI_Controller{
	public function __construct(){ 
		parent::__construct();
		if($this->session->login['role'] != 'super' && $this->session->login['role'] != 'produksi') redirect();
		$this->data['aktif'] = 'MasterHasil';
		$this->load->model('produksi/M_MasterHasil', 'm_master_hasil');
	}

	public function index(){
		$this->data['title'] = 'Semua Hasil';
		$this->data['all_barang'] = $this->m_master_hasil->lihat(); 
		$this->data['no'] = 1;
		
		$this->load->view('produksi/dataMaster/hasil/master_hasil', $this->data);
	}
	
	public function tambah()
	{
		$this->data['title'] = 'Tambah Hasil';
		$dariDB = $this->m_master_hasil->cekkodebarang();
		$nourut = substr($dariDB, 2, 6);
		$kodeBarangSekarang = $nourut + 1;
		$this->data['kode_komponen'] = $kodeBarangSekarang;
		$this->data['all_barang'] = $this->m_master_hasil->AllBarang();

		// Get the nama_tindakan options from the model
		$this->load->model('m_master_hasil');
		$this->data['tindakan_options'] = $this->m_master_hasil->get_tindakan_options();

		$this->load->view('produksi/dataMaster/hasil/tambah_hasil', $this->data);
	}
	...
	public function proses_tambah(){ 
		$data = [ 
			...
			'total_assy' => $this->input->post('jumlah_assembly'),
			'id_tindakan' => $this->input->post('id_tindakan'),
		];

		if($this->m_master_hasil->tambah($data)){
			$this->session->set_flashdata('success', 'Data Barang <strong>Berhasil</strong> Ditambahkan!');
			redirect('produksi/MasterHasil');
		} else {
	...
	{ 
			...
			// Load the Produksi_model to get the tindakan options
			$this->load->model('m_master_hasil');
			$this->data['tindakan_options'] = $this->m_master_hasil->get_tindakan_options();
			$this->data['title'] = 'Ubah Barang';
			$this->data['barang'] = $this->m_master_hasil->lihat_id($kode_komponen);


			$this->load->view('produksi/dataMaster/hasil/ubahhasil', $this->data);
	}
	...
	public function proses_ubah($komponen){ 
		$data = [ 
			'kode_alat' => $this->input->post('kode_alat'),
			'nama_alat' => $this->input->post('nama_alat'),
			'total_assy' => $this->input->post('total_assy'), 
			'id_tindakan' => $this->input->post('id_tindakan'),
		];

		if($this->m_master_hasil->ubah($data, $komponen)){
	...
	public function hapus($kode_alat){ 
		...
		if($this->m_master_hasil->hapus($kode_alat)){
