<?php 
class NamaPengambil extends CI_Controller{
	...
	public function index(){
		...
		$this->data['all_barang'] = $this->m_barang_elektro->lihatKaryawan(); 
		$this->data['no'] = 1;
		
		$this->load->view('purchasingElektro/manajemenPengambil/lihat', $this->data);
	}

	public function tambahKaryawan(){ 
		$this->data['title'] = 'Tambah Barang HFNC';
		$dariDB = $this->m_barang_elektro->cekId_karyawan(); 
        // $nourut = substr($dariDB, 1, 3);
		$kodeBarangSekarang = $dariDB + 1;
		$this->data['kode_komponen'] = $kodeBarangSekarang;
		// $this->data['all_barang'] = $this->m_barang_elektro->AllBarang();

		$this->load->view('purchasingElektro/manajemenPengambil/tambah_karyawan', $this->data);
	}

	public function proses_tambahKaryawan(){ 
		$data = [ 
			'id' => $this->input->post(''),
			'nama_karyawan' => $this->input->post('nama_karyawan'),
		];

		if($this->m_barang_elektro->tambahKaryawan($data)){
			$this->session->set_flashdata('success', 'Data Barang <strong>Berhasil</strong> Ditambahkan!');
			redirect('PurchasingElektro/NamaPengambil');
		} else {
	...
	public function ubah_karyawan($kode_komponen){ 

		$this->data['title'] = 'Ubah Nama Karyawan';
		$this->data['barang'] = $this->m_barang_elektro->lihat_idKaryawan($kode_komponen); 

		$this->load->view('purchasingElektro/manajemenPengambil/ubah_karyawan', $this->data);
	}

	public function proses_ubahKaryawan($komponen){ 

		$data = [ 
			'nama_karyawan' => $this->input->post('nama_karyawan'), 
		];

		if($this->m_barang_elektro->ubahKaryawan($data, $komponen)){
	...
	public function hapus_Karyawan($komponen){ 
		
		if($this->m_barang_elektro->hapusKaryawan($komponen)){
