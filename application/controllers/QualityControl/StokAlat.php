...
class StokAlat extends CI_Controller{
	...
	public function index(){
		...
		$this->load->view('qualityControl/hasil/lihat', $this->data);
	}
	...
	public function tambah(){ 
		...
		if(empty($dariDB)){
    		...
    		$this->load->view('qualityControl/hasil/tambah_alat', $this->data);
		}else{
	        $dariDB = $this->m_stok_alat->cekkodetransaksi();
	...
	public function proses_tambah(){
		...
		if(empty($countDuplicate)){ 
			$data_terima = [
				...
				redirect('QualityControl/StokAlat'); 
		} else { 
			$this->session->set_flashdata('errorStok', 'Tanggal Sudah Ada !'); 
			redirect('QualityControl/StokAlat/tambah');  
		} 
	}
	...
	public function tambahEdit($kode_transaksi){    
		...
		$this->load->view('qualityControl/hasil/tambahEdit', $this->data);
	}
	...
	public function keranjang_barang(){
		$this->load->view('qualityControl/hasil/keranjang');
	} 
	...
	public function proses_tambahEdit(){  
		...
		for($i = 0; $i < $jumlah_barang_diterima; $i++){
			...
			$data_detail_terima[$i]['total_accept'] = $this->input->post('total_accept_hidden')[$i];
			$data_detail_terima[$i]['total_reject'] = $this->input->post('total_reject_hidden')[$i];
		...
		if($this->m_stok_alat->tambahDetail($data_detail_terima)){
			for ($i=0; $i < $jumlah_barang_diterima ; $i++) { 
				$this->m_stok_alat->plus_stok($data_detail_terima[$i]['total_accept'], $data_detail_terima[$i]['kode_alat']) or die('gagal min stok');
			}
	...
	public function detail($kode_transaksi){
		...
		$this->load->view('qualityControl/hasil/detail', $this->data);
	}
	...
	public function detailHapus($kode_komponen)
	{
		...
		$this->load->view('qualityControl/hasil/hapusDetail', $this->data);
	}

	public function proses_detailHapus()
    { 
        $id_detail = $this->input->post('id_detail');
        $total_stok = $this->input->post('total_accept');
