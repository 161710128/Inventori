<?php
class M_InventarisElektro extends CI_Model {
	protected $_table = 'elektro_inventaris';  
    ...
    {
        $query = $this->db->query("SELECT MAX(tanggal) as tanggal from elektro_inventaris");
        $hasil = $query->row();
    ...
    {
        $query = $this->db->query("SELECT MAX(kode_transaksi) as kodetransaksi from elektro_inventaris");
        $hasil = $query->row();
