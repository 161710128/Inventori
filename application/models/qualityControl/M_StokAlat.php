...
class M_StokAlat extends CI_Model{
	protected $_table = 'qc_masuk';

	protected $_table1 = 'qc_alat';
	protected $_table2 = 'qc_detail_masuk';
    ...
    {
        $query = $this->db->query("SELECT MAX(kode_transaksi) as kodetransaksi from qc_masuk");
        $hasil = $query->row();
	...
	public function plus_stok($total_stok, $kode_komponen){ 
		$query = $this->db->set('total_accept', 'total_accept+' . $total_stok, false);
		$query = $this->db->where('kode_alat', $kode_komponen); 
	...
	public function lihat_no_terima_detail($kode_transaksi){ 
		return $this->db->query("SELECT * from qc_detail_masuk WHERE kode_transaksi='$kode_transaksi' ORDER BY nama_shift")->result(); 
	}
	...
	public function stok_alat(){
		$query = $this->db->query("SELECT * FROM qc_alat");
        $hasil = $query->result();
        return $hasil;
	}
