...
class M_HasilKerja extends CI_Model{
	protected $_table = 'produksi_masuk';
	protected $_table1 = 'produksi_master_barang';
	protected $_table2 = 'produksi_detail_masuk';
	...
	public function lihat(){
		...
		// return $query->result();
		$query = $this->db->query("SELECT * FROM produksi_masuk ORDER BY tanggal");
        $hasil = $query->result();
        return $hasil;
	}

	public function jumlahProgresss(){
		$query = $this->db->get($this->_table);
		return $query->num_rows();
	}

	public function Stok_timbanganDewasa(){
	...
	public function plus_stok($total_stok, $kode_komponen, $total_rijek, $id_barang){
		$query = $this->db->set('total_rijek', 'total_rijek+' . $total_rijek, false);
    ...
    {
        $query = $this->db->query("SELECT MAX(kode_transaksi) as kodetransaksi from produksi_masuk");
        $hasil = $query->row();
	...
	public function lihat_no_terima_detail($kode_transaksi){ 
		// return $this->db->query("SELECT * from produksi_detail_masuk WHERE kode_transaksi='$kode_transaksi' ORDER BY nama_shift")->result();
	...
	{
		// $query = $this->db->get_where($this->_table2, ['id_detailmasuk' => $kode_komponen]);
		// return $query->row();
		...
		return $this->db->query("SELECT produksi_detail_masuk.id_detailmasuk, produksi_detail_masuk.kode_transaksi, produksi_detail_masuk.kode_job, 
								...
								INNER JOIN produksi_detail_masuk
								ON produksi_barang.id_barang = produksi_detail_masuk.id_barang
								WHERE id_detailmasuk='$kode_komponen' 
								ORDER BY nama_shift")->row();
	}


	public function hapusDetail($kode_transaksi)
	{
		return $this->db->delete($this->_table2, ['id_detailmasuk' => $kode_transaksi]);
	}
