...
<body class="hold-transition sidebar-mini layout-fixed">
                                        ...
                                        <div class="card-header">
                                            <!-- <h3 class="card-title">
                                                ...
                                                <ul aria-labelledby="dropdownSubMenu1" class="dropdown-menu border-0 shadow"> 
                                                    <li><a href="<?= base_url('GudangBarangJadi/Pengeluaran/tambahEdit_hfnc/' . $barang->kode_transaksi) ?>" class="dropdown-item">HFNC</a></li> 
                                                    <li><a href="<?= base_url('GudangBarangJadi/Pengeluaran/tambahEdit_antropometri/' . $barang->kode_transaksi) ?>" class="dropdown-item">Antropometri</a></li> 
                                                </ul>
                                            </h3>   -->
                                            <div class="float-right"><a href="<?= base_url('GudangBarangJadi/Pengeluaran') ?>" class="btn btn-secondary btn-sm"><i class="fa fa-reply"></i>&nbsp;&nbsp;Kembali</a></div>
                                        </div>
                                        <div class="card-body">
                                            <form action="<?= base_url('GudangBarangJadi/Pengeluaran/proses_tambahEdit') ?>" id="form-tambah" method="POST">
                                                <h5>Data Pengguna</h5>
                                                <hr>
                                                <div class="form-row">
                                                        ...
                                                        <div class="form-group col-2">
                                                            <label for="kode_komponen" >Nama Barang</label>
                                                                <select name="kode_komponen" id="kode_komponen" class="form-control">
                                                                    <option value="" class="custom-label">Pilih</option>
                                                                    <?php foreach ($all_barangHFNC as $komponen) : ?>
                                                                        <option value="<?= $komponen->kode_komponen ?>"><?= $komponen->nama_komponen ?>  <?= $komponen->spesifikasi ?></option>
                                                                    <?php endforeach ?>
                                                                </select>
                                                            </div>
                                                            <div class="form-group col-2">
                                                            ...
                                                            <div class="form-group col-2">
                                                                ...
                                                                <input type="text" name="pengambil" value="" class="form-control" >
															</div> 
                                                            <div class="form-group col-1">
    ...
    <style>
    .custom-label {
        /* Set the desired height for the label here */
        /* For example, to set the height to 40px, you can use: */
        height: 100px;
    }
</style>
    <!-- /.content-wrapper -->
       ...
       $(document).ready(function() {
            ...
            $('#kode_komponen').on('change', function() { 
                if ($(this).val() == '') reset()
                    ...
                    $.ajax({
                        url: "<?= base_url() ?>GudangBarangJadi/Pengeluaran/get_all_barang",
                        type: 'POST',
                        dataType: 'json',
                        data: {
                        ...
                        success: function(data) {
                            ...
                            $('input[name="pengambil"]').prop('disabled', false)
                            $('select[name="shift"]').prop('disabled', false)  
            ...
            $(document).on('click', '#tambah', function(e) {
                ...
                const data_keranjang = {
                    ...
                    pengambil: $('input[name="pengambil"]').val(),
                ...
                $.ajax({
                    url: "<?= base_url() ?>GudangBarangJadi/Pengeluaran/keranjang_barang",
                    type: 'POST',
                    data: data_keranjang,
                    success: function(data) {
            ...
            function reset() {
                $('#kode_komponen').val([]).trigger('change');
                ...
                $('input[name="pengambil"]').val('')
                $('input[name="pengambil"]').prop('disabled', true)
                $('select[name="shift"]').prop('disabled', true)
